...
like setting up tools like Webpack, Babel, and TypeScript. Next.js takes care of
all that for us, so that we can focus on diving into building our app. Next gives
us features like Routing, Server-Side Rendering, Static Site Generation, and Data
Fetching, all out of the box. Without it, we'd have to install and setup a lot of
different packages to get the same functionality. I use Next.js to build web dashboards
for my Discord bots thanks to its developer experience and performance. Next integrates
like a charm with tRPC, allowing us that sweet end-to-end type safety.
